...
# DAG 정의
with DAG(
        dag_id="hdhs_procedure_verifi_daily_d-3",  # DAG의 고유 식별자
    ...
    def procedure_verification(**kwargs):
        start_time = execution_time.format("YYYY-MM-DD 17:50:00")
        start_time_end = execution_time.format("YYYY-MM-DD 20:29:00")
        start_time_d2 = execution_time.subtract(days=3).format("YYYY-MM-DD 00:10:00")
        start_time_d2_end = execution_time.subtract(days=3).format("YYYY-MM-DD 08:00:00")
        ...
        print(procedure_list_query)

        snowflake_cursor.execute(procedure_list_query)
        procedure_list = [(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9]) for row in snowflake_cursor.fetchall()]
            ...
            try:
                ...
                if procedure[4]:
                    ...
                    ora_query = f"""
                            ...
                            WHERE PGMID = '{procedure[0]}' 
                            AND JBPMT = '{procedure[4]}'
                            AND STARTTIME >= TO_TIMESTAMP('{start_time_d2}', 'YYYY-MM-DD HH24:MI:SS')
                            AND STARTTIME < TO_TIMESTAMP('{start_time_d2_end}', 'YYYY-MM-DD HH24:MI:SS')
                            ORDER BY STARTTIME ASC
                            """
                else:
                    snow_query = f"""
                ...
                if not snow_results or not ora_results:
                    raise ValueError(f"No data found for DB: [{flag}] PGMID: {procedure[0]}, JBPMT: {procedure[4]}")
                ...
                for snow_result, ora_result in zip(snow_results, ora_results):
                    ...
                    PGMID = snow_result[0]
                    ORA_START_TIME = "'"+ora_result[1].strftime('%Y-%m-%d %H:%M:%S')+"'" if ora_result[1] else 'Null'
                    ...
                    ORA_ST = ora_result[3]
                    SNOW_START_TIME = "'"+snow_result[1].strftime('%Y-%m-%d %H:%M:%S')+"'" if snow_result[1] else 'Null'
                    ...
                    isrt_query = f"""
                                INSERT INTO DW_ETL_DB.DW_ETC.ORA_SNOW_DATA_PIPE_VAL_d2
                                VALUES ('{SMR_DT}',
                                        '{PGMID}',
                                        {ORA_START_TIME},
                                        {ORA_END_TIME},
                ...
                SMR_DT = execution_time.format("YYYY-MM-DD")
                PGMID = procedure[0]
                SNOW_START_TIME = "'" + procedure[1].strftime('%Y-%m-%d %H:%M:%S') + "'" if procedure[1] else 'Null'
                SNOW_END_TIME = "'" + procedure[2].strftime('%Y-%m-%d %H:%M:%S') + "'" if procedure[2] else 'Null'
                SNOW_EXEC_TIME = ((procedure[2] or datetime.datetime.min) - (
                        procedure[1] or datetime.datetime.min)).total_seconds()
                SNOW_ST = procedure[3]
                JBPMT = procedure[4]
                READ_CNT_ORA = procedure[5] or 0
                BYCNT_ORA = procedure[6] or 0
                ERRCNT_ORA = procedure[7] or 0
                UPDCNT_ORA = procedure[8] or 0
                WRTCNT_ORA = procedure[9] or 0

                # 오류 발생 시 에러 정보 저장
                error_query = f"""
                        INSERT INTO DW_ETL_DB.DW_ETC.ORA_SNOW_DATA_PIPE_VAL_d2 (SMR_DT, PGMID, SNOW_START_TIME, SNOW_END_TIME,SNOW_ST, JBPMT, READ_CNT_ORA, BYCNT_ORA, ERRCNT_ORA, UPDCNT_ORA, WRTCNT_ORA, ERR_MSG, SNOW_EXEC_TIME)
                        VALUES ('{SMR_DT}', 
                                '{PGMID}',
                                {SNOW_START_TIME},
                                {SNOW_END_TIME},
