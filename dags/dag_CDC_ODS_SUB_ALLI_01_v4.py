        ...
        with postgres_hook.get_conn() as postgres_conn:
            ...
            while True:
                ...
                copy_into_query = f"""
                    ...
                    FROM @{STAGE_NAME}/{schema}/{table_name}/{date_folder}/{fm_p_end.strftime('%Y%m%d')}-batch{batch_number}-{time_identifier}.parquet
                    FILE_FORMAT = (TYPE = PARQUET);
                """
                snowflake_cursor.execute(copy_into_query)
                print(f"Data copied into {temp_table} from stage.")

                snowflake_cursor.execute(f"REMOVE @{STAGE_NAME}/{schema}/{table_name}/{date_folder}/{fm_p_end.strftime('%Y%m%d')}-batch{batch_number}-{time_identifier}.parquet;")

                print("임시 테이블 데이터 로드 성공!")

                print(f"Temporary file removed from stage.")
...
# Define the DAG
with DAG(
    dag_id="dag_CDC_ODS_SUB_ALLI_01_v4",
