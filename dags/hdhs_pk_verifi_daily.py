...
def get_filtered_table_list(snowflake_hook):
    query = """
        ...
        WHERE TABLE_NAME IN (
                            'CU_SVMT_GRNT_DTL',
                            'MK_PRMO_TRGT_ITEM_PTC_DTL',
                            'MK_EVNT_ENTRY_DTL',
                            'IM_SLITM_PRC_DTL',
                            'CU_CUST_APP_INF',
                            'MK_UNTD_PRMO_SVMT_INF',
                            'MK_PRMO_ENTRY_DTL',
                            'OD_STLM_INF_CRYPT',
                            'IM_ETC_MNG_SLITM_MST',
                            'BD_BFMT_INF',
                            'AM_ALML_ORD_MPNG_DTL',
                            'MK_EVNT_INF',
                            'OD_ORD_DTL',
                            'MK_PRMO_TRGT_CUST_ORD_HIS',
                            'IM_SLITM_MST',
                            'OD_HPNT_PAY_APRVL_DTL_CRYPT',
                            'CU_CUST_MST',
                            'MK_SVMT_PRMO_GRNT_DTL',
                            'OM_DISP_ITEM_DTL',
                            'OD_BASKT_INF'
                            )
    """
...
# DAG 정의
with DAG(
    dag_id="hdhs_pk_verifi_daily",  # DAG의 고유 식별자
    ...
    def ora_push():
        ...
        start_days_ago = pendulum.now(kst).subtract(days=3).start_of('day')
        ...
        #고유 batch_id 생성
        batch_id = f"daily_{pendulum.now(kst).format('YYYYMMDDHHmmss')}"

        #고유 batch_id 변수 저장
        Variable.set("batch_id", batch_id)
        ...
        for table, columns in table_list.items():
            try:
                ...
                # 종료 시간 이후 데이터 확인(SNOW_ONLY에서 제외 용도)
                ...
                chg_ext_ora_endtime_after_df.rename(columns={"TO_CHAR(CHG_DTM,'YYYYMMDDHH24MISS')": "CHG_DTM"}, inplace=True)
                if not chg_ext_ora_endtime_after_df.empty:
                    temp_max_chg_dtm_endtime_after = pd.to_datetime(chg_ext_ora_endtime_after_df['CHG_DTM']).max().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                else:
                ...
                batch_size = 150000
                for batch_df_endtime_after in split_dataframe(chg_ext_ora_endtime_after_df, batch_size):
                    batch_df_endtime_after.to_sql(f"TEMP_{table_name}_ORA_ENDTIME_AFTER", con=engine, schema="VERIFI_DATA", if_exists='append',
                                    index=False)
    ...
    trigger_hdhs_pk_error_merge = TriggerDagRunOperator(
        task_id='trigger_hdhs_pk_error_merge',
        trigger_dag_id='hdhs_pk_error_merge',
    ...
    ora_push() >> trigger_hdhs_pk_error_merge
