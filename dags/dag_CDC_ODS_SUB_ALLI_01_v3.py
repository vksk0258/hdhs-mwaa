...
def process_in_batches(table, query, schema, table_name,columns, pk_columns, **kwargs):
    ...
    with postgres_hook.get_conn() as postgres_conn:
        ...
        with snowflake_connection.cursor() as snowflake_cursor:
            ...
            batch_size = 20000  # 한 번에 실행할 최대 행 수

            chunk_index = 1

            for i in range(0, len(values), batch_size):
                temp_table = f"{schema}.{table_name}{chunk_index}"
                ...
                snowflake_cursor.executemany(insert_query, batch)
                # 3️⃣ MERGE 실행
...
# Define the DAG
with DAG(
    dag_id="dag_CDC_ODS_SUB_ALLI_01_v3",
