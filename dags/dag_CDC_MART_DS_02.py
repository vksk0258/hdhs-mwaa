...
def log_etl_completion(**kwargs):
    ...
    print(f"*** {complete_time} : CDC_MART_MART_DW_02 프로시져 실행 완료 **")

with DAG(
    dag_id="dag_CDC_MART_DS_02",
    schedule_interval=None,
    tags=["현대홈쇼핑",'dag_CDC_MART_01']
) as dag:
    task_SP_RPD_DLV_FORM_DLU_SMR = PythonOperator(
        task_id="task_SP_RPD_DLV_FORM_DLU_SMR",
        python_callable=execute_procedure,
        op_args=["SP_RPD_DLV_FORM_DLU_SMR", p_start, p_end],
    ...
    task_SP_RPD_DLV_FORM_OSHP_DLU_FCT = PythonOperator(
        ...
        python_callable=execute_procedure,
        op_args=["SP_RPD_DLV_FORM_OSHP_DLU_FCT", p_start, p_end],
        trigger_rule="all_done"
    )
