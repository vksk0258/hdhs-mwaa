...
def incremental_load(table, **kwargs):
    ...
    if table_name != "TEC_CONT_CORP_CRYPT":
        query = f"""
            SELECT * FROM {table}
            WHERE CHG_DTM >= TO_DATE('{previous_time_str}', 'YYYY-MM-DD HH24:MI:SS')
            AND CHG_DTM < TO_DATE('{current_time_str}', 'YYYY-MM-DD HH24:MI:SS')
            """
    else:
        query = f"""
            SELECT * FROM {table}
            WHERE MODIFY_DATE >= TO_DATE('{previous_time_str}', 'YYYY-MM-DD HH24:MI:SS')
...
dag = DAG(
    ...
    start_date=pendulum.datetime(2024, 12, 16, 8, 0, 0, tz="Asia/Seoul"),
    schedule_interval="0 * * * *",
