...
# DAG 정의
with DAG(
        dag_id="dag_CDC_ODS_SUB_CMS_01_S3",
    ...
    def task_DWCT_DAGENT_c_01(**kwargs):
        TMP_DIR = "/tmp/CMS_incremental"
        ...
        df['ETL_DTM'] = pendulum.now(KST).strftime('%Y-%m-%d %H:%M:%S')
        ...
        if not df.empty:
            ...
            s3_client.upload_file(file_name, S3_BUCKET_NAME,
                                  f"dw/{schema}/{table_name}/{date_folder}/{file_name.split('/')[-1]}")
            os.remove(file_name)
        else:
            print(f"No data to process for table {schema}.{table_name} in the given time window.")


    @task(task_id='task_DWCT_HAGENT_c_01')
    def task_DWCT_HAGENT_c_01(**kwargs):
        TMP_DIR = "/tmp/CMS_incremental"
        current_time = kwargs['data_interval_end'].in_tz(KST)
