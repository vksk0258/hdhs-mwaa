...
key = "param/wf_DD01_0400_DAU_SMS_01.json"
...
# DAG 정의
with DAG(
        dag_id="dag_CDC_ODS_DAU_SMS_01",  # DAG의 고유 식별자
        schedule=None,  # 예약 일정 없음 (수동 실행)
        catchup=False,  # 과거 데이터 실행 스킵
        dagrun_timeout=datetime.timedelta(minutes=6000),  # DAG 실행 제한 시간
        tags=["현대홈쇼핑", "ODS"]  # DAG에 붙일 태그
) as dag:


    @task(task_id='task_A_DAU_SMS_c_01')
    def task_A_DAU_SMS_c_01 ():
        import os
        import pandas as pd
        import numpy as np
