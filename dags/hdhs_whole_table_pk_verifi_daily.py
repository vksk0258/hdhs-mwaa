...
batch_id = f"whole_table_daily_{pendulum.now(kst).format('YYYYMMDDHH')}"
        ...
        try:
            ...
            with snowflake_hook.get_conn() as snowflake_conn:
                with snowflake_conn.cursor() as cursor:
                    ...
                    CREATE OR REPLACE TABLE DW_LOAD_DB.VERIFI_DATA.{log_table_name} (
                    ...
                    if diff_results:
                        ...
                        VALUES {', '.join(values)}
                        """
                        print("diff_results : ", insert_log_query)
                    ...
                    create_outer_log_table_query = f"""
                        CREATE OR REPLACE TABLE DW_LOAD_DB.VERIFI_DATA.{outer_log_table_name} (
                            BATCH_ID VARCHAR(50), {', '.join([f'{col} VARCHAR(50)' for col in columns])},
                    ...
                    if only_ora_results:
                        ...
                        for row in only_ora_results:
                            formatted_row = ", ".join([f"'{value}'" for value in row])
                            values.append(f"('{batch_id}',{formatted_row}, '{row}', NULL, '{start_time}')")
                        insert_outer_log_query = f"""
                        INSERT INTO DW_LOAD_DB.VERIFI_DATA.{outer_log_table_name}
                        VALUES {', '.join(values)}
                        """
                        print("only_ora_results : ", insert_outer_log_query)
                    ...
                    if only_snow_results:
                        ...
                        for row in only_snow_results:
                            formatted_row = ", ".join([f"'{value}'" for value in row])
                            values.append(f"('{batch_id}',{formatted_row}, NULL, '{row}', '{start_time}')")
                        insert_outer_log_query = f"""
                        INSERT INTO DW_LOAD_DB.VERIFI_DATA.{outer_log_table_name}
                        VALUES {', '.join(values)}
                        """
                        print("only_snow_results : ", insert_outer_log_query)
                        cursor.execute(insert_outer_log_query)
...
# DAG 정의
with DAG(
    dag_id="hdhs_whole_table_pk_verifi_daily",  # DAG의 고유 식별자
    start_date=pendulum.datetime(2025, 2, 26, tz="Asia/Seoul"),
    ...
    # 병렬 실행된 모든 태스크가 끝난 후 트리거 실행
    parallel_tasks >> trigger_hdhs_whole_table_pk_error_merge