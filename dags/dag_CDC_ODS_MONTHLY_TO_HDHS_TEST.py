...
with DAG(
    ...
    def task_CU_TCS_CUST_HIS_TO_HDHS_pre_sql(p_start,p_end,ora_main_conn_id):
        pre_sql = f"""
        DELETE FROM HDHS_CU.CU_TCS_CUST_HIS
    ...
    def task_CU_ITNT_CUST_GRD_INF_TMP_i_01_TO_HDHS_DEV(p_start):
        ...
        with snow_hook.get_conn() as snow_connection:
            with ora_main_connection.cursor() as ora_main_cursor:
                ...
                # 변수 설정
                last_cust_no = None  # 첫 번째 페이지는 초기값으로 None
                batch_read_size = 500000
                offset = 0
                ...
                while True:
                    ...
                    if last_cust_no is None:
                        ...
                        SELECT * 
                        FROM {snow_table}
                        WHERE CUST_NO > '{last_cust_no}'  -- 이전 페이지의 마지막 CUST_NO
                        ORDER BY CUST_NO
                        LIMIT {batch_read_size}
                        """

                    print(f"==== [SPLIT QUERY] OFFSET: {offset} ====")
                    print(paged_query)
                    df = pd.read_sql(paged_query, snow_connection)

                    if df.empty:
                        break

                    # 마지막 CUST_NO를 업데이트 (마지막 행에서 CUST_NO 값 가져오기)
                    last_cust_no = df.iloc[-1]['CUST_NO']
