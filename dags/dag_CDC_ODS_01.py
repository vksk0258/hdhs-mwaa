...
with DAG(
    ...
    trigger_dag_CDC_ODS_SUB_HES = TriggerDagRunOperator(
        task_id='trigger_dag_CDC_ODS_SUB_HES',
        trigger_dag_id='dag_CDC_ODS_SUB_HES',
        reset_dag_run=True,         # 이미 수행된 dag여도 수행 할 것인지
        wait_for_completion=True,  # 트리거 하는 dag가 끝날때까지 기다릴 것인지
        poke_interval=60,
        allowed_states=['success'], # 트리거 하는 dag가 어떤 상태여야 오퍼레이터가 성공으로 끝나는지
    ...
    [trigger_dag_CDC_ODS_SUB_TMS_01,trigger_dag_CDC_ODS_SUB_HES]